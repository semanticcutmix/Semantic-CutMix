...
class CAM(object):
    ...
    def forward(self, x, show, idx):
        ...
        if show:
            return cam,idx,prob[0,idx]
        return cam, idx

    def __call__(self, x, show = False, idx = None):
        return self.forward(x, show, idx)
...
def get_CAM(image, wrapped_model, image_size):
     
    cam, _ = wrapped_model(image)
    cam = F.interpolate(cam, size=(image_size, image_size), mode='bilinear', align_corners=False)
    cam = cam.squeeze()
    cam = cam * (image_size*image_size/torch.sum(cam))
    # print('yes')
    return cam